...
import { madridJanuaryCalendar, madridSchoolNonTeachingDays, type SchoolNonTeachingDay } from './services/madridSchoolCalendar';
...
function dateKey(calendarYear: number, month: number, day: number) { return `${calendarYear}-${String(month + 1).padStart(2,'0')}-${String(day).padStart(2,'0')}`; }
function monthMarkup(calendarYear: number, month: number, nextYearJanuary = false) {
  const first = new Date(calendarYear, month, 1); const count = new Date(calendarYear, month + 1, 0).getDate();
  const monthPrefix = `${calendarYear}-${String(month + 1).padStart(2, '0')}`;
  ...
  const januaryCalendar = nextYearJanuary ? madridJanuaryCalendar(calendarYear) : undefined;
  ...
  for (let day = 1; day <= count; day++) {
    const date = dateKey(calendarYear, month, day); const recurringName = recurringHolidayName(date); const entry = recurringName ? undefined : data.days[date]; const holiday = (januaryCalendar?.holidays ?? data.holidays).find(h => h.date === date); const schoolDay = januaryCalendar?.schoolDays.find(item => item.date === date); const schoolOverlayDay = schoolCalendarVisible && !nextYearJanuary ? schoolNonTeachingDays.find(item => item.date === date) : undefined; const weekend = isWeekend(date);
    let label = ''; let accessibleLabel = ''; let style = ''; let cls = weekend ? 'weekend' : '';
    if (schoolDay) { cls += ` ${schoolDay.kind}`; label = schoolDay.kind === 'school-break' ? 'Vacaciones escolares' : 'No lectivo'; accessibleLabel = schoolDay.name; }
    ...
    if (schoolOverlayDay) { cls += ' school-non-teaching'; accessibleLabel = `${accessibleLabel ? `${accessibleLabel}, ` : ''}${schoolOverlayDay.name}, no lectivo`; }
  ...
  const heading = nextYearJanuary ? `${months[month]} ${calendarYear}` : months[month];
  const detail = nextYearJanuary ? `Solo <strong>V60</strong> · saldo ${year}` : `Teletrabajo: <strong>${teleworkDays}</strong> ${teleworkDays === 1 ? 'día' : 'días'}`;
...
function render() {
  ...
  shell(`<main><section class="summary"><div><p class="eyebrow">SALDOS ANUALES</p><h2>Tu año, de un vistazo</h2></div>${countersMarkup()}</section><div class="calendar-tools"><button class="secondary school-calendar-button ${schoolCalendarVisible ? 'active' : ''}" id="school-calendar" aria-pressed="${schoolCalendarVisible}">${schoolCalendarVisible ? 'Ocultar calendario lectivo' : 'Cargar calendario lectivo de Madrid'}</button>${schoolCalendarVisible ? '<p>Vista temporal: los días no lectivos aparecen con un marco rayado y no se guardan en tu calendario.</p>' : ''}</div><section class="calendar">${months.map((_,m) => monthMarkup(year, m)).join('')}${monthMarkup(year + 1, 0, true)}</section><section class="legend" aria-label="Leyenda">${data.dayTypes.map(t => `<span><i class="lg" style="background:${t.color}"></i>${esc(t.name)}</span>`).join('')}<span><i class="lg holiday-national"></i>Festivo nacional</span><span><i class="lg holiday-regional"></i>Comunidad de Madrid</span><span><i class="lg holiday-local"></i>Ciudad de Madrid</span><span><i class="lg holiday-recurring"></i>Festivo fijo</span><span><i class="lg school-break"></i>Vacaciones escolares</span><span><i class="lg non-teaching"></i>Día no lectivo</span><span><i class="lg weekend"></i>Fin de semana</span>${schoolCalendarVisible ? '<span><i class="lg school-non-teaching"></i>No lectivo (calendario escolar completo)</span>' : ''}</section></main>`);
...
async function cycleDay(date: string) {
  const code = nextDayTypeCode(data.dayTypes, data.days[date]?.code, date, year);
...
function editCode(index?: number) {
  ...
  modal.innerHTML = `<div class="backdrop"><form class="sheet"><button type="button" class="close">×</button><h2>${old ? 'Editar' : 'Nuevo'} código</h2><label>Código<input name="code" maxlength="8" required value="${esc(old?.code ?? '')}"></label><label>Descripción<input name="name" required value="${esc(old?.name ?? '')}"></label><label>Límite anual<input name="limit" type="number" min="0" required value="${old?.limit ?? 1}"></label><label>Fecha límite de uso<input name="useUntil" type="date" min="${year}-01-01" max="${year + 1}-12-31" required value="${old?.useUntil ?? defaultUseUntil(year, '')}"></label><label>Color<input name="color" type="color" value="${old?.color ?? '#d97555'}"></label><button class="primary">Guardar</button></form></div>`;
  modal.querySelector('.close')!.addEventListener('click', () => modal.innerHTML = ''); modal.querySelector('form')!.onsubmit = async e => { e.preventDefault(); const form = new FormData(e.currentTarget as HTMLFormElement); const item: DayType = { code: String(form.get('code')).trim().toUpperCase(), name: String(form.get('name')).trim(), limit: Number(form.get('limit')), color: String(form.get('color')), useUntil: String(form.get('useUntil')) }; if (data.dayTypes.some((t,i) => t.code === item.code && i !== index)) return alert('Ese código ya existe.'); if (old && old.code !== item.code) Object.values(data.days).forEach(d => { if (d.code === old.code) d.code = item.code; }); if (index === undefined) data.dayTypes.push(item); else data.dayTypes[index] = item; await repository.saveYear(data); renderSettings(); };
}
