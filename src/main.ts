...
function monthMarkup(month: number) {
  ...
  for (let day = 1; day <= count; day++) {
    const date = dateKey(month, day); const recurringName = recurringHolidayName(date); const entry = recurringName ? undefined : data.days[date]; const holiday = data.holidays.find(h => h.date === date); const schoolDay = schoolDays.get(date); const weekend = isWeekend(date);
...
function render() {
  ...
  shell(`<main><section class="summary"><div><p class="eyebrow">SALDOS ANUALES</p><h2>Tu año, de un vistazo</h2></div>${countersMarkup()}</section><section class="school-calendar-tools"><div><strong>Calendario lectivo de Madrid</strong><small>Consulta visual; no modifica ni guarda tus días.</small></div><button class="secondary ${schoolDays.size ? 'active' : ''}" id="school-calendar">${schoolDays.size ? 'Ocultar días no lectivos' : 'Cargar días no lectivos'}</button></section><section class="calendar">${months.map((_,m) => monthMarkup(m)).join('')}</section><section class="legend" aria-label="Leyenda">${data.dayTypes.map(t => `<span><i class="lg" style="background:${t.color}"></i>${esc(t.name)}</span>`).join('')}<span><i class="lg holiday-national"></i>Festivo nacional</span><span><i class="lg holiday-regional"></i>Comunidad de Madrid</span><span><i class="lg holiday-local"></i>Ciudad de Madrid</span><span><i class="lg holiday-recurring"></i>Festivo fijo</span><span><i class="lg weekend"></i>Fin de semana</span>${schoolDays.size ? '<span><i class="lg school-non-teaching"></i>No lectivo (colegios e institutos)</span>' : ''}</section></main>`);
