...
function monthMarkup(month: number) {
  ...
  for (let day = 1; day <= count; day++) {
    const date = dateKey(month, day); const recurringName = recurringHolidayName(date); const entry = recurringName ? undefined : data.days[date]; const holiday = data.holidays.find(h => h.date === date); const weekend = isWeekend(date);
