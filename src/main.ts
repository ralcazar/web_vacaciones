...
import { madridJanuaryCalendar, madridJanuaryNonTeachingDays, madridSchoolNonTeachingDays, type SchoolNonTeachingDay } from './services/madridSchoolCalendar';
...
function monthMarkup(calendarYear: number, month: number, nextYearJanuary = false) {
  ...
  for (let day = 1; day <= count; day++) {
    const date = dateKey(calendarYear, month, day); const recurringName = recurringHolidayName(date); const entry = recurringName ? undefined : data.days[date]; const holiday = (januaryCalendar?.holidays ?? data.holidays).find(h => h.date === date); const schoolOverlayDay = schoolCalendarVisible ? schoolNonTeachingDays.find(item => item.date === date) : undefined; const weekend = isWeekend(date);
...
function toggleSchoolCalendar() {
  ...
  try { schoolNonTeachingDays = [...madridSchoolNonTeachingDays(year), ...madridJanuaryNonTeachingDays(year + 1)]; schoolCalendarVisible = true; render(); }
