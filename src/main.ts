...
import type { CalendarEntry, HolidayScope, LeaveType, YearData } from './domain/models';
...
function monthMarkup(month: number) {
  ...
  for (let day = 1; day <= count; day++) {
    ...
    if (holiday) { cls += ' holiday'; label = holiday.name; }
    if (entry?.type === 'telework') { cls += ' telework'; label = 'Teletrabajo'; }
    if (entry?.type === 'leave') { const type = data.leaveTypes.find(t => t.code === entry.code); cls += ' leave'; label = entry.code; style = `--entry:${type?.color ?? '#d97555'}`; }
...
function countersMarkup() { return calculateCounters(toConfig(data), entriesOf(data)).map(c => `<div class="counter ${c.exceeded ? 'exceeded' : ''}"><span class="dot" style="background:${c.color ?? '#d97555'}"></span><strong>${esc(c.code === 'telework' ? c.name : c.code)}</strong><span>${c.used} / ${c.limit}</span><small>${c.remaining >= 0 ? `${c.remaining} restantes` : `${Math.abs(c.remaining)} de más`}</small></div>`).join('') || '<p class="muted">Añade códigos en Configuración.</p>'; }
...
function render() {
  ...
  shell(`<main><section class="summary"><div><p class="eyebrow">RESUMEN ANUAL</p><h2>Tu año, de un vistazo</h2></div><div class="counters">${countersMarkup()}</div></section><section class="legend"><span><i class="lg telework"></i>Teletrabajo</span><span><i class="lg leave"></i>Ausencia</span><span><i class="lg holiday"></i>Festivo</span><span><i class="lg weekend"></i>Fin de semana</span></section><section class="calendar">${months.map((_,m) => monthMarkup(m)).join('')}</section></main>`);
...
function openDay(date: string) {
  ...
  modal.innerHTML = `<div class="backdrop"><section class="sheet"><button class="close">×</button><p class="eyebrow">${date}</p><h2>¿Cómo fue este día?</h2>${blocked ? `<p class="warning">⚠️ ${holiday ? `Es festivo: ${esc(holiday.name)}` : 'Es fin de semana'}. Puedes asignarlo, pero no es un día laborable.</p>` : ''}<div class="options"><button data-kind="normal">Día normal</button><button data-kind="telework">Teletrabajo</button>${data.leaveTypes.map(t => `<button data-kind="leave" data-code="${esc(t.code)}"><i style="background:${t.color ?? '#d97555'}"></i>${esc(t.code)} · ${esc(t.name)}</button>`).join('')}</div></section></div>`;
  modal.querySelector('.close')!.addEventListener('click', () => modal.innerHTML = '');
  modal.querySelectorAll<HTMLButtonElement>('[data-kind]').forEach(b => b.onclick = async () => { if (b.dataset.kind === 'normal') { await repository.deleteDay(date); delete data.days[date]; } else { const entry = b.dataset.kind === 'telework' ? { date, type: 'telework' } as CalendarEntry : { date, type: 'leave', code: b.dataset.code! } as CalendarEntry; await repository.saveDay(entry); const { date: _, ...day } = entry; data.days[date] = day; } render(); });
}
function renderSettings() {
  shell(`<main class="settings"><section><p class="eyebrow">CONFIGURACIÓN DE ${year}</p><h2>Límites y códigos</h2><label>Máximo anual de teletrabajo<input id="tw-limit" type="number" min="0" value="${data.settings.teleworkLimit}"></label><div class="list">${data.leaveTypes.map((t,i) => `<div class="list-row"><i style="background:${t.color ?? '#d97555'}"></i><div><strong>${esc(t.code)}</strong><small>${esc(t.name)} · Límite ${t.limit}</small></div><button data-edit="${i}">Editar</button><button class="danger" data-remove="${i}" aria-label="Eliminar">×</button></div>`).join('')}</div><button class="primary" id="add-code">+ Añadir código</button></section><section><p class="eyebrow">FESTIVOS</p><h2>Fechas no laborables</h2><div class="list">${data.holidays.sort((a,b) => a.date.localeCompare(b.date)).map((h,i) => `<div class="list-row"><div><strong>${h.date.slice(5)}</strong><small>${esc(h.name)} · ${scopeName(h.scope)}</small></div><button class="danger" data-remove-holiday="${i}">×</button></div>`).join('') || '<p class="muted">Todavía no hay festivos.</p>'}</div><button class="secondary" id="add-holiday">+ Añadir festivo</button></section><section><p class="eyebrow">COPIA DE SEGURIDAD</p><h2>Importar y exportar</h2><p class="muted">El JSON contiene todo el año y no depende del futuro proveedor de datos.</p><div class="actions"><button class="secondary" id="export">Exportar JSON</button><label class="button secondary">Importar JSON<input id="import" type="file" accept="application/json"></label></div><p class="dev-note">Modo de desarrollo: repositorio en memoria. Los datos se reinician al recargar.</p></section></main>`);
  app.querySelector<HTMLInputElement>('#tw-limit')!.onchange = async e => { data.settings.teleworkLimit = Math.max(0, Number((e.target as HTMLInputElement).value)); await saveConfig(); renderSettings(); };
  ...
  app.querySelectorAll<HTMLButtonElement>('[data-remove]').forEach(b => b.onclick = async () => { const t = data.leaveTypes[Number(b.dataset.remove)]!; if (Object.values(data.days).some(d => d.type === 'leave' && d.code === t.code)) return alert('No puedes eliminar un código que tiene días asignados.'); data.leaveTypes.splice(Number(b.dataset.remove),1); await saveConfig(); renderSettings(); });
...
const scopeName = (s: HolidayScope) => ({national:'Nacional',regional:'Comunidad de Madrid',local:'Collado Villalba'}[s]);
async function saveConfig() { await repository.saveYearConfig(toConfig(data)); }
function editCode(index?: number) {
  const old = index === undefined ? undefined : data.leaveTypes[index]; const modal = app.querySelector<HTMLDivElement>('#modal')!;
  ...
  modal.querySelector('.close')!.addEventListener('click', () => modal.innerHTML = ''); modal.querySelector('form')!.onsubmit = async e => { e.preventDefault(); const form = new FormData(e.currentTarget as HTMLFormElement); const item: LeaveType = { code: String(form.get('code')).trim().toUpperCase(), name: String(form.get('name')).trim(), limit: Number(form.get('limit')), color: String(form.get('color')) }; if (data.leaveTypes.some((t,i) => t.code === item.code && i !== index)) return alert('Ese código ya existe.'); if (old && old.code !== item.code) Object.values(data.days).forEach(d => { if (d.type === 'leave' && d.code === old.code) d.code = item.code; }); if (index === undefined) data.leaveTypes.push(item); else data.leaveTypes[index] = item; await repository.saveYear(data); renderSettings(); };
}
...
async function handleImport(e: Event) { const file = (e.target as HTMLInputElement).files?.[0]; if (!file) return; try { const imported = importYear(await file.text()); const current = await repository.getYear(imported.year); const summary = `${imported.year}: ${imported.leaveTypes.length} códigos, ${imported.holidays.length} festivos y ${Object.keys(imported.days).length} días.`; if (!confirm(`Archivo válido. ${summary}\n${current ? 'Ya existen datos de ese año. ¿Quieres sobrescribirlos?' : '¿Quieres importarlo?'}`)) return; await repository.saveYear(imported); year = imported.year; data = imported; renderSettings(); } catch (err) { alert(err instanceof Error ? err.message : 'No se pudo importar.'); } }
